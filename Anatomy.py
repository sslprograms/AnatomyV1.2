...
init()
hwid = subprocess.getoutput('wmic csproduct get uuid').split('UUID')[1].strip()
...
def check_robux(cookie, prox):
    try:
        with requests.session() as session:
            ...
            robux = session.get(f'https://economy.roblox.com/v1/users/{userid}/currency', proxies=proxy, timeout=15)
...
def select_visit_bot():
    ...
    while True:
        try:
            ...
            with requests.session() as session:
                session.cookies['.ROBLOSECURITY'] = cookie
                session.headers['x-csrf-token'] = session.post('https://catalog.roblox.com/v1/catalog/items/details').headers['x-csrf-token']
...
def set_online(cookie):
    try:
        with requests.session() as session:
            ...
            prox = random.choice(proxies)
            proxy = {'http':prox, 'https':prox}
            session.headers['x-csrf-token'] = session.get('https://www.roblox.com/home', timeout=15).content.decode('utf8').split("Roblox.XsrfToken.setToken('")[1].split("');")[0]
...
def send_report(cookie, prox, userid):
    try:
        with requests.session() as session:
            ...
            report = session.post(f'https://www.roblox.com/abusereport/userprofile?id={userid}&redirecturl=https%3a%2f%2fwww.roblox.com%2fusers%2f{userid}%2fprofile', proxies=proxy, data={'__RequestVerificationToken':__RequestVerificationToken, 'ReportCategory':1, 'Comment':'', 'Id':userid, 'RedirectUrl':f'https://www.roblox.com/abusereport/userprofile?id={userid}', 'PartyGuid':'', 'ConversationId':''}, timeout=15)
...
[19]: Report bot  / [20]: Ally bot   / [21]: Game Attack
    ''')
    print(Fore.BLUE + '████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████')
